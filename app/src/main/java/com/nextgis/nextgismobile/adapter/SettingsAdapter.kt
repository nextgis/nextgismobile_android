...
class SettingAdapter(val items: List<Setting>) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) = (holder as ViewHolder).bind(items[position])

    override fun getItemCount(): Int = items.size

    class ViewHolder(private var binding: ItemSettingBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(repo: Setting) {
